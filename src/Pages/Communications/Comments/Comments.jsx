...
export default function CommentsList() {
    ...
    return (
     <div className="grid grid-cols-2 gap-3  mx-auto p-4  dark:bg-dark-bg-surface dark:text-light-bg-surface" dir="rtl">
            {comments.map((ticket) => (
                <div key={ticket.id} className={`flex flex-col justify-between items-start gap-4 p-4 rounded-lg border ${ticket.read ? "bg-gray-50" : "bg-white shadow-md"} duration-200 dark:bg-dark-bg-surface dark:text-light-bg-surface dark:border-dark-border-light dark:hover:border-dark-border-focus ease-in-out cursor-pointer`}>
                    ...
                    <div className="mt-3 flex justify-end gap-2">
                        ...
                        >
                            تایید
                              ...
                              <button
                            onClick={() => onConfirm(ticket.id)}
                            className="px-3 py-1 text-sm font-medium text-dark-text-primary bg-red-600 rounded hover:bg-red-700 transition-colors"
                        >
                         رد
                        </button>
